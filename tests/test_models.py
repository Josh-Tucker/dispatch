...
class TestRssFeed:
    ...
    def test_create_rss_feed_with_required_fields(self, test_session: Session, sample_feed_data: dict):
    ...
    def test_get_unread_count_empty_feed(self, test_session: Session, sample_feed: RssFeed):
        """Test getting unread count when feed has no entries."""
        count = sample_feed.get_unread_count(test_session)
        assert count == 0

    def test_get_unread_count_with_mixed_entries(self, test_session: Session, sample_feed: RssFeed):
...
class TestRssEntry:
    ...
    def test_create_rss_entry_with_all_fields(self, test_session: Session, sample_feed: RssFeed, sample_entry_data: dict):
    ...
    def test_create_rss_entry_minimal_fields(self, test_session: Session, sample_feed: RssFeed):
    ...
    def test_rss_entry_default_read_status(self, test_session: Session, sample_feed: RssFeed):
        """Test that RSS entries default to unread status."""
        entry = RssEntry(
            feed_id=sample_feed.id,
            title='Test Entry',
            guid='default-read-test'
        )
        test_session.add(entry)
        test_session.commit()
    ...
    def test_rss_entry_feed_relationship(self, test_session: Session, sample_feed: RssFeed):
    ...
    def test_multiple_entries_same_feed(self, test_session: Session, sample_feed: RssFeed):
