...
from dispatch.models.model import RssEntry, RssFeed, Settings
from dispatch.services.entry_service import get_all_feed_entries
from dispatch.services.feed_service import get_all_feeds, get_feed_by_id
from dispatch.services.theme_service import get_default_theme, set_default_theme
...
class TestFeedService:
    ...
    def test_get_feed_by_id_existing_feed(
    ...
    def test_get_all_feeds_returns_feeds_list(
...
class TestEntryService:
    ...
    def test_get_all_feed_entries_returns_list(
        self, test_session: Session, multiple_entries: list[RssEntry]
    ):
    ...
    def test_get_all_feed_entries_ordered_by_published(
        self, test_session: Session, multiple_entries: list[RssEntry]
    ):
        ...
        if len(entries) > 1:
            ...
            for i in range(len(entries) - 1):
                current_published = entries[i].published
                next_published = entries[i + 1].published
                if current_published is not None and next_published is not None:
...
class TestServiceIntegration:
    ...
    def test_feed_and_entry_relationship(
        self,
        ...
        feed_entries = [
            entry for entry in all_entries if entry.feed_id == sample_feed.id
        ]
