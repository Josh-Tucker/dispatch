...
def fetch_full_article_route(entry_id):
    ...
    try:
        ...
        if not entry:
            return (
                '<div class="fetch-error-message"><span style="color: #dc3545;">✗ Entry not found</span></div>',
                404,
            )

        if not entry.link:
            return (
                '<div class="fetch-error-message"><span style="color: #dc3545;">✗ No link available for this entry</span></div>',
                400,
            )
        ...
        if article:
            ...
            return (
                f'<div class="fetch-error-message"><span style="color: #dc3545;">✗ Failed to fetch content from {entry.link}</span><br><small>The website may be blocking requests or the content may not be accessible.</small></div>',
                500,
            )

    except requests.exceptions.Timeout:
        return (
            '<div class="fetch-error-message"><span style="color: #dc3545;">✗ Request timed out</span><br><small>The website took too long to respond.</small></div>',
            500,
        )
    except requests.exceptions.ConnectionError:
        return (
            '<div class="fetch-error-message"><span style="color: #dc3545;">✗ Connection failed</span><br><small>Could not connect to the website.</small></div>',
            500,
        )
    except Exception as e:
...
def serve_favicon(feed_id):
    ...
    try:
        ...
        if feed and feed.favicon_data:
            response = Response(
                feed.favicon_data, mimetype=feed.favicon_mime_type or "image/x-icon"
            )
